...
def configure():
    global subject_set_id, feedback_id, training_subject, image_folder_path, excel_file_path, \
        sim_folder_path, csv_file_name, csv_file_path, metadata_fields, wb, ws, first_empty_row

    # get image folder and excel manifest file paths
    image_folder_path = input('Enter the folder path: ')
    excel_file_path = input('Excel file path: ')

    # image_folder_path = r"C:\Users\dkess\OneDrive\Documents\CWRU\Macro\Data, Analysis\Slab 1 12x16 no flash - Copy\processed_Slab 1 12x16 no flash - Copy"
